...
if(isset($_GET['id'])){
    ...
    if(isset($_POST['update'])){
        ...
        $phone = $_POST['phone'];
        $class = $_POST['class'];
        $stmt=$conn->prepare("update student set name='$name',phone='$phone',class_id='$class' where id=$id");
        $stmt->execute();
        ...
        <div class="u-form u-form-1">
            <form method="post" type="form" action="<?=$_SERVER['PHP_SELF']?>" class="u-clearfix u-form-spacing-20 u-form-vertical " style="padding: 10px" >
                ...
                <div class="u-form-group u-form-name u-label-none">
                    ...
                    <input type="text" value="<?=$row['name']?>" placeholder="Enter your Name" id="studentName" name="studentName" class="u-border-2 u-border-black u-border-no-left u-border-no-right u-border-no-top u-input u-input-rectangle ">
                    <input type="hidden" value="<?=$id?>" placeholder="Enter your Name" id="id" name="id" class="u-border-2 u-border-black u-border-no-left u-border-no-right u-border-no-top u-input u-input-rectangle ">
                </div>
                <div class="u-form-group u-form-name u-label-none">
                    <label for="name-3b9a" class="u-label">phone</label>
                    <input type="tel" value="<?=$row['phone']?>" placeholder="Enter your phone" id="phone" name="phone" class="u-border-2 u-border-black u-border-no-left u-border-no-right u-border-no-top u-input u-input-rectangle ">
                </div>
                    ...
                    <div class="u-form-select-wrapper">
                        <select id="class" name="class" class="u-border-2 u-border-black u-border-no-left u-border-no-right u-border-no-top u-input u-input-rectangle ">
                            ...
                            foreach ($select as $row){
                                ?>
                                <option value="<?=$row['id']?>" <?php if($row['id']==$id){echo 'selected';}?> data-calc="1"><?php echo $row['name']?></option>
                ...
                <div class="u-align-center u-form-group ">
                    <input type="submit"  name="update" value="update" class=" u-border-2 u-border-black u-btn u-btn-rectangle u-button-style u-button-submit u-btn-1">
                </div>
